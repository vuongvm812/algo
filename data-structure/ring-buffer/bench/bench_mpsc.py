"""Benchmark for the batch MPSC ring buffer in mpsc.py.

Run: python3 bench/bench_mpsc.py [-n N] [--size SIZE] [--batch B] [--repeat R]
                                 [--producers P]

Scenarios (items/s counts individual items, not batches):
  insert          single producer, N items in batches of B, buffer never fills
                  (measures the cost of the uncontended lock in insert())
  insert+emit     fill to capacity in batches of B, emit(), repeat
                  (single thread)
  spsc-threaded   1 producer / 1 consumer, producer retries rejected suffix
  mpsc-threaded   P producers / 1 consumer, N items split evenly across
                  producers, each retrying its rejected suffix
  deque baseline  collections.deque doing the same work (extend / drain);
                  the threaded deque variant guards every call with a Lock
"""
import argparse
import os
import sys
...
from mpsc import Data, RingBuffer  # noqa: E402
...
def bench_insert(n, size, batch, producers):
    rb = RingBuffer(n + 1)
...
def bench_insert_emit(n, size, batch, producers):
    rb = RingBuffer(size)
    cap = size - 1
    batch = min(batch, cap)
...
def bench_threaded(n, size, batch, producers):
    rb = RingBuffer(size)
    batch = min(batch, size - 1)
    payloads = []
    for pid, count in enumerate(split(n, producers)):
        payloads.append([Data((pid, i)) for i in range(count)])
    remaining = [producers]
...
def bench_spsc_threaded(n, size, batch, producers):
    return bench_threaded(n, size, batch, 1)


def bench_deque_insert(n, size, batch, producers):
    dq = deque()
    batches = chunks([Data(i) for i in range(n)], batch)
    extend = dq.extend
...
def bench_deque_insert_emit(n, size, batch, producers):
    dq = deque()
    cap = size - 1
    batch = min(batch, cap)
    fills = chunks([Data(i) for i in range(n)], cap)
...
def bench_deque_threaded(n, size, batch, producers):
    """Bounded deque + one Lock around every call: the naive MPSC baseline."""
    dq = deque()
    lock = threading.Lock()
    cap = size - 1
    batch = min(batch, cap)
...
def main():
    ...
    ap.add_argument("--producers", type=int, default=4, help="producer threads for mpsc-threaded")
    args = ap.parse_args()
    ...
    scenarios = [
        ...
        (f"RingBuffer mpsc-threaded x{args.producers}", bench_threaded),
        ("deque insert", bench_deque_insert),
        ("deque insert+emit", bench_deque_insert_emit),
        (f"deque+Lock mpsc-threaded x{args.producers}", bench_deque_threaded),
    ]

    print(
        f"n={args.n:,} size={args.size} batch={args.batch} producers={args.producers}"
        f" best-of-{args.repeat}  (Python {sys.version.split()[0]})"
    )
    print(f"{'scenario':34} {'time (s)':>9} {'items/s':>13} {'ns/item':>8} {'drained':>10}")
    for name, fn in scenarios:
        best, drained = min(
            (fn(args.n, args.size, args.batch, args.producers) for _ in range(args.repeat)),
            key=lambda r: r[0],
        )
        ops = args.n / best
